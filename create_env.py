...
class Environment(object):
   ...
   def add_resource(self, name, _type, required, optional_keys, depends=None, **kwargs):
      ...
      if _type =="AWS::AutoScaling::AutoScalingGroup":
         temp_resource = Resource(_type,
                  required,
                  optional_keys,
                  Tags=[{"Key":"Name", "Value": self.cf_join([self.ref("AWS::StackName"), name]), "PropagateAtLaunch":True }],
                  **kwargs)
      elif "Tags" in optional_keys and "Tags" not in kwargs:
         temp_resource = Resource(_type,
                  required,
                  optional_keys,
                  Tags=self.default_tags + [{"Key":"Name", "Value": self.cf_join([self.ref("AWS::StackName"), name])}],
                  **kwargs)
      elif "Tags" in optional_keys:
         temp_resource = Resource(_type,
                  required,
                  optional_keys,
                  Tags=self.default_tags + [{"Key":"Name", "Value": name}] + kwargs.pop("Tags"),
                  **kwargs)
      else:
         temp_resource = Resource(_type,
                  required,
   ...
   def add_vpc(self, name, cidr_block="192.168.0.0/16", **kwargs):
      ...
      if not self.default_network: 
         ...
         self.network_generator = self.default_network.subnet(self.subnet_default)
   ...
   def attach_internet_gateway(self, name, vpc=None, subnets=None, gateway=None):
      gateway = self.get_first("AWS::EC2::InternetGateway") if not gateway else gateway
      vpc = self.get_first("AWS::EC2::VPC") if not vpc else vpc
      required = {
                  "VpcId": dict,
   ...
   def add_route_table(self, name, vpc=None, attached=None):
      vpc = self.ref(self.get_first("AWS::EC2::VPC")) if not vpc else self.ref(vpc)
      required = {"VpcId": dict}
      optional = {"Tags": list}
      attached = self.get_first("AWS::EC2::VPCGatewayAttachment") if not attached else attached
      self.add_resource(name,
                        "AWS::EC2::RouteTable",
                        required,
                        optional,
   ...
   def add_route(self, name, cidr_block, route_table=None, depends=[],  **kwargs):
      route_table = self.get_first("AWS::EC2::RouteTable") if not route_table else route_table
   ...
   def add_launch_configuration(self, name, image,instance_type, vpc=None, security_groups=None, **kwargs):
      vpc = self.get_first("AWS::EC2::VPC") if not vpc else vpc
      security_groups = [self.ref(s) for s in self.get_all("AWS::EC2::SecurityGroup")] if not security_groups else [self.ref(s) for s in security_groups]
      vpc_ref = self.ref(vpc)
      required = {
                  "ImageId": str,
                  "InstanceType": str
   ...
   def add_autoscaling_group(self, name,max_size="1", min_size="0", subnets=None, instance=None, launch_config=None, **kwargs):
      ...
      if not instance:
         launch_config = self.get_first("AWS::AutoScaling::LaunchConfiguration") if not launch_config else launch_config
      required = {
                   "MaxSize": str,
                   "MinSize": str
                 }
...
class SecurityGroupRules(object):
   ...
   def __init__(self, _type):
      ...
      if self.type not in self.options:
         self.exception("Type set incorecctly must be in {}".format(self.options))
   def add_rule(self, ip_protocol, cidr_ip=None, from_port=None, to_port=None, source_security_group_id=None, source_security_group_name=None, destination_security_group_id=None):
      ...
      if self.type == "SecurityGroupIngress" and destination_security_group_id:
         self.exception("Ingress and destiniation_security_group_id incompatible")
      elif self.type == "SecurityGroupEgress" and source_security_group_name or source_security_group_id:
         self.exception("Engress and source_security_group_id/name incompatible")   
      for key in raw_rule:
         if raw_rule[key] and key in ("FromPort", "ToPort"):
            if type(raw_rule[key]) != int:
               self.exception("{} should be int, got {}".format(key, type(raw_rule[key])))
         elif raw_rule[key]:
            if type(raw_rule[key]) != str:
               self.exception("{} should be str, got {}".format(key, type(raw_rule[key])))
      self.rules.append({key: raw_rule[key] for key in raw_rule if raw_rule[key]})
...
if __name__ == "__main__":
   my_env = Environment()
   my_env.add_vpc("VPC")
   my_env.add_subnet("My first subnet", AvailabilityZone={ "Fn::Select": [ "1", {"Fn::GetAZs": {"Ref": "AWS::Region"}} ]})
   my_env.add_subnet("My second subnet", AvailabilityZone={ "Fn::Select": [ "2", {"Fn::GetAZs": {"Ref": "AWS::Region"}} ]})
   ...
   my_env.add_subnet_to_route_table("add second subnet", subnet="MySecondSubnet")
   ...
   my_env.add_security_group("My security group", in_rules.rules, out_rules.rules) 
   my_env.add_launch_configuration("my launch configuration", "ami-64385917" ,"t2.micro")
